...
int main(int argc, char* argv[])
{
	 ...
	 */
	while ((opt = getopt(argc, argv, "b:s:c:i:j:k:l:m:n:h")) != -1) {
		switch (opt) {
			case 'b': /* bank bit */
			...
			case 'i': /* iterations */
				...
				break;
			case 'j':
				entry_shift[0] = (uint64_t)strtol(optarg, NULL, 0);
				break;
			case 'k':
				entry_shift[1] = (uint64_t)strtol(optarg, NULL, 0);
				break;
			case 'l':
				entry_shift[2] = (uint64_t)strtol(optarg, NULL, 0);
				break;
			case 'm':
				entry_shift[3] = (uint64_t)strtol(optarg, NULL, 0);
				break;
			case 'n':
				entry_shift[4] = (uint64_t)strtol(optarg, NULL, 0);
				break;
		}

	}
	
	//for(i = 0; i < 5; i ++) printf("entry_dist[%d]:%lu\n", i, entry_shift[i]);
